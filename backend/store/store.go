...
type HealthStore interface {
	GetClusterHealth(ctx context.Context, cluster clientv3.Cluster, etcdClientTLSConfig *tls.Config) *corev2.HealthResponse
}
...
type SilencedStore interface {
	...
	// returned if none were found.
	GetSilencedEntries(ctx context.Context) ([]*corev2.Silenced, error)
	...
	// returned if none were found.
	GetSilencedEntriesByCheckName(ctx context.Context, check string) ([]*corev2.Silenced, error)

	// GetSilencedEntriesBySubscription returns all entries for the given subscription
	// within the ctx's namespace. A nil slice with no error is
	// returned if none were found.
	GetSilencedEntriesBySubscription(ctx context.Context, subscriptions ...string) ([]*corev2.Silenced, error)

	// GetSilencedEntryByName returns an entry using the given id and the
	// namespace stored in ctx. The resulting entry is nil if
	// none was found.
	GetSilencedEntryByName(ctx context.Context, id string) (*corev2.Silenced, error)

	// UpdateSilencedEntry creates or updates a given entry.
	UpdateSilencedEntry(ctx context.Context, entry *corev2.Silenced) error

	// GetSilencedEntriesByName gets all the named silenced entries.
	GetSilencedEntriesByName(ctx context.Context, id ...string) ([]*corev2.Silenced, error)
}
