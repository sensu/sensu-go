...
func TestAdapterV1_getFilterForResource(t *testing.T) {
	type fields struct {
		Store           store.Store
		StoreTimeout    time.Duration
		FilterAdapters  []FilterAdapter
	...
	tests := []struct {
		...
		{
			name: "returns an error when no filter adapters exist",
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := a.getFilterForResource(tt.args.ctx, tt.args.ref)
			if (err != nil) != tt.wantErr {
				t.Errorf("AdapterV1.getFilterForResource() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("AdapterV1.getFilterForResource() = %v, want %v", got, tt.want)
			}
		})
	}
}
