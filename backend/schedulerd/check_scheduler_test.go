...
func newIntervalScheduler(t *testing.T, ctx context.Context, executor string) *TestIntervalScheduler {
	...
	scheduler.scheduler = NewIntervalScheduler(ctx, s, scheduler.msgBus, scheduler.check, &cache.ResourceCacher{})

	assert.NoError(scheduler.msgBus.Start())

	switch executor {
	case "adhoc":
		scheduler.exec = NewAdhocRequestExecutor(ctx, s, &queue.Memory{}, scheduler.msgBus, &cache.ResourceCacher{})
	default:
		scheduler.exec = NewCheckExecutor(scheduler.msgBus, "default", s, &cache.ResourceCacher{})
	}
...
func newCronScheduler(t *testing.T, ctx context.Context, executor string) *TestCronScheduler {
	...
	scheduler.scheduler = NewCronScheduler(ctx, s, scheduler.msgBus, scheduler.check, &cache.ResourceCacher{})
