...
func newIntervalScheduler(ctx context.Context, t *testing.T, executor string) *TestIntervalScheduler {
	...
	scheduler.scheduler = NewIntervalScheduler(ctx, s, scheduler.msgBus, scheduler.check, &cachev3.Resource{}, pm)

	assert.NoError(scheduler.msgBus.Start())

	switch executor {
	case "adhoc":
		scheduler.exec = NewAdhocRequestExecutor(ctx, s, &queue.Memory{}, scheduler.msgBus, &cachev3.Resource{}, pm)
	default:
		scheduler.exec = NewCheckExecutor(scheduler.msgBus, "default", s, &cachev3.Resource{}, pm)
	}
...
func newCronScheduler(ctx context.Context, t *testing.T, executor string) *TestCronScheduler {
	...
	scheduler.scheduler = NewCronScheduler(ctx, s, scheduler.msgBus, scheduler.check, &cachev3.Resource{}, pm)
