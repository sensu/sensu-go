...
func TestEventsRouter(t *testing.T) {
	...
	fixtureNoNamespace := corev2.FixtureEvent("foo", "check-cpu")
	fixtureNoNamespace.Entity.ObjectMeta.Namespace = ""
...
type mockEventCreateController struct {
	mock.Mock
}

func (m *mockEventCreateController) CreateOrReplace(ctx context.Context, check *corev2.Event) error {
	return m.Called(ctx, check).Error(0)
}

func TestEventsCreateRouter(t *testing.T) {
	type controllerFunc func(*mockEventCreateController)

	// Setup the router
	controller := &mockEventCreateController{}
	router := EventsCreateRouter{controller: controller}
	parentRouter := mux.NewRouter().PathPrefix(corev2.URLPrefix).Subrouter()
	router.Mount(parentRouter)
	...
	tests := []struct {
		...
		},
		{
			...
			controllerFunc: func(c *mockEventCreateController) {
		...
		},
		{
			...
			method: http.MethodPut,
			path:   fixture.URIPath(),
			body:   marshal(fixtureNoNamespace),
			controllerFunc: func(c *mockEventCreateController) {
				c.On("CreateOrReplace", mock.Anything, mock.Anything).
					Return(nil).
					Once()
