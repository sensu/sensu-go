...
func (p *Pipelined) expandHandlers(handlers []string, level int) (map[string]*types.Handler, error) {
	...
	for _, handlerName := range handlers {
		...
		if handler == nil {
			if err != nil {
				log.Error("pipelined failed to retrieve a handler: ", err.Error())
			} else {
				log.Error("pipelined failed to retrieve a handler: name= ", handlerName)
			}
			continue
		}
		...
		if handler.Type == "set" {
			...
			if err != nil {
				log.Error("pipelined failed to expand handler set: ", err.Error())
			} else {
...
func (p *Pipelined) pipeHandler(handler *types.Handler, eventData []byte) (*command.Execution, error) {
	...
	if err != nil {
		log.Error("pipelined failed to execute event pipe handler: ", err.Error())
	} else {
		log.Errorf("pipelined executed event pipe handler: status=%x output=%s", result.Status, result.Output)
	}
...
func (p *Pipelined) socketHandler(handler *types.Handler, eventData []byte) (net.Conn, error) {
	...
	if err != nil {
		log.Errorf("pipelined failed to execute event %s handler: %v", protocol, err.Error())
	} else {
		log.Errorf("pipelined executed event %s handler: bytes=%v", protocol, bytes)
	}
