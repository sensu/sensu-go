...
test("RelativeDate given direction prop", () => {
  const now = new Date();
  const date = new Date(now.getTime() + 3500);

  let elem = mount(
    <RelativeDate to={now} dateTime={date.toUTCString()} direction="pending" />,
  );
  expect(elem.text()).toEqual("in a few seconds");

  elem = mount(
    <RelativeDate to={now} dateTime={date.toUTCString()} direction="past" />,
  );
  expect(elem.text()).toEqual("just now");
});
