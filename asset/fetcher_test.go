...
func TestFetchExistingAsset(t *testing.T) {
	...
	fetcher := &httpFetcher{
		URLGetter: func(ctx context.Context, path, trustedCAFile string, header map[string]string, proxy string, proxy_disabled bool) (io.ReadCloser, error) {
			return os.Open(path)
		},
	}
	f, err := fetcher.Fetch(context.TODO(), localAssetPath, headers, "", false)
	if err != nil {
...
func TestHTTPGet(t *testing.T) {
	...
	closer, err := httpGet(context.Background(), ts.URL, "", headers, "", false)
