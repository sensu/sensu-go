...
func TestResolveOpampAgentConfig(t *testing.T) {
	var value interface{} = new(OpampAgentConfig)
	if _, ok := value.(Resource); ok {
		resource, err := ResolveResource("OpampAgentConfig")
		if err != nil {
			t.Fatal(err)
		}
		meta := resource.GetMetadata()
	...
	_, err := ResolveResource("OpampAgentConfig")
	if err == nil {
		t.Fatal("expected non-nil error")
	}
	if got, want := err.Error(), `"OpampAgentConfig" is not a Resource`; got != want {
		t.Fatalf("unexpected error: %s", err)
	}
}

func TestResolveOpampAgentConfigByRBACName(t *testing.T) {
	value := new(OpampAgentConfig)
	var iface interface{} = value
	resource, err := ResolveResourceByRBACName(value.RBACName())
	if _, ok := iface.(Resource); ok {
		if err != nil {
...
func TestResolveOpampAgentConfigByStoreName(t *testing.T) {
	value := new(OpampAgentConfig)
	var iface interface{} = value
	resource, err := ResolveResourceByStoreName(value.StoreName())
	if _, ok := iface.(Resource); ok {
...
func TestResolveV2ResourceOpampAgentConfig(t *testing.T) {
	v2Resource, err := ResolveV2Resource("OpampAgentConfig")
	if err != nil {
		t.Fatal(err)
	}
	v3Resource, err := ResolveResource("OpampAgentConfig")
	if err != nil {
		t.Fatal(err)
	}
	if got, want := v2Resource.(*V2ResourceProxy).Resource, v3Resource; !reflect.DeepEqual(got, want) {
