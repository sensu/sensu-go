...
func TestInfoCommandRunEClosure(t *testing.T) {
	...
	client.On("FetchSilenced", mock.Anything).Return(corev2.FixtureSilenced("foo:bar"), nil)
...
func TestInfoCommandRunEClosureWithErr(t *testing.T) {
	...
	cli := test.NewCLI()
	client := cli.Client.(*client.MockClient)
	client.On("FetchSilenced", mock.Anything).Return(&corev2.Silenced{}, errors.New("my-err"))
